...
export class ProductImporter {
    ...
    public async all(conn: Connection) {       
        ...
        for (let productName of productNameList){
            let prdAttrsTarget = await Queries.queryProductAttributeIds(conn, productName);
            let prdIdsTarget = await Queries.queryProductIds(conn, productName);
            let allStdPricebookEntriesTarget = await Queries.queryStdPricebookEntryIds(conn, productName);
            let allPricebookEntriesTarget = await Queries.queryPricebookEntryIds(conn, productName);
    ...
    for(let dirName of dirNames){
        if(dirName=== 'Standard Price Book' ){continue;}
...
for(let stdPbe of this.stdPbes){
    ...
    private async deleteBulkObject(conn: Connection, sObjectName: string, data: string[]): Promise<string> {
